...
class PO
{
    ...
    public function list_purchased_orders()
    {
		...
		if(isset($_REQUEST['search']))
		{
			...
			if(!empty($_REQUEST['po_no'])  || !empty($_REQUEST['vendor_name'])  || !empty($_REQUEST['po_date']) || !empty($_REQUEST['sort_po']))
			{
				$whereArr = "";
  	...
  	$('.fancybox').on('click',function(e){
		...
		var page_value = $('#page_value').val();
           // $('.fancybox').fancybox();
			...
			$('.commission_change').live('click',function(e){
			e.preventDefault();
...
$commision_per = "";
	...
	?>
		...
		<?php 
			...
			<?php  $ref_query = "SELECT * FROM ".$wpdb->prefix.'po_transaction'." where order_id='".$order_val->order_id."' and product_id='".$order_val->product_id."'order by date_paid desc limit 1";
			...
			<td class="ref_id" id="<?php echo $order_val->order_id; ?>" data-id="<?php echo $order_val->product_id; ?>"><?php echo $ref[0]->ref_no; ?></td>
		<?php	 $po = $this->calculate_po($order_val->order_id,$order_val->product_id);  ?>
			...
			<td><a href="<?php  echo admin_url() ; ?>admin.php?page=purchased-orders&view=order&order_id=<?php echo $order_val->order_id ; ?>&product_id=<?php echo $order_val->product_id ; ?>">View Detail</a><br/><a href="<?php  echo admin_url() ; ?>admin.php?page=purchased-orders&action=send_email&order_id=<?php echo $order_val->order_id ; ?>&product_id=<?php echo $order_val->product_id ; ?>&vendor_id=<?php echo $order_val->vendor_id ; ?>">Send Email</a></td>
...
<?php
    ...
    color: #fff !important;
		...
		<div class="contentpanel">
		<div class="row" style="padding:10px;"><div style="float:left;"><img src="<?php echo get_site_url(); ?>/wp-content/uploads/2015/08/memorabiliaexpertLOGO.jpg"/></div><div style="float:right;padding-20px;"><p style="font-weight:bold;">BidAMI-Memorabilia.Expert <br/> 6490 W. Desert Inn <br/> Las Vegas NV 89146</p></div></div>
		</div>
  ...
  <h3 class="panel-title" style="display:inline">Payments and Adjustments</h3>
		...
		var page_value = $('#page_value').val();
           // $('.fancybox').fancybox();
			...
			$('#com_button').live('click',function(e){
			e.preventDefault();
		...
		if($_REQUEST['action'] == 'send_email' && !empty($_REQUEST['order_id']) && !empty($_REQUEST['product_id']))
		{
			...
			$url = admin_url().'admin.php?page=purchased-orders&msg=success';
...
<html>
	...
	<body>
	<div class="contentpanel">
		<div class="row" style="padding:10px;"><div style="float:left;"><img src="'.get_site_url().'/wp-content/uploads/2015/08/memorabiliaexpertLOGO.jpg"/></div><div style="float:right;padding-20px;"><p style="font-weight:bold;">BidAMI-Memorabilia.Expert <br/> 6490 W. Desert Inn <br/> Las Vegas NV 89146</p></div></div>
		</div>
